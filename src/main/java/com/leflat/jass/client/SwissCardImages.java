...
public class SwissCardImages {
    private final static Logger LOGGER = Logger.getLogger(SwissCardImages.class.getName());
    ...
    static {
        for (int i = 0; i < Card.DECK_SIZE; i++) {
            var imagePath = SwissCardImages.class.getClassLoader().getResource(imagePath(i));
            if (imagePath == null) {
        ...
        var backImagePath = SwissCardImages.class.getClassLoader().getResource(IMG_PATH + "back.png");
        ...
        for (int i = 0; i < 4; i++) {
            var colorImagePath = SwissCardImages.class.getClassLoader().getResource(IMG_PATH + "c" + i + ".png");
